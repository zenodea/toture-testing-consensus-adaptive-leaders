...
type StragglerAttack_4 struct {
	logger *util.Logger
}

func NewStragglerAttack_4(logger *util.Logger) *StragglerAttack_4 {
	return &StragglerAttack_4{
		logger: logger,
	}
}

func (a *StragglerAttack_4) Attack(nodes []*AttackNode, links [][]*AttackLink, oracle *LeaderOracle, duration int) {
	fmt.Printf("Running straggler attack 4 majority stragglers  for %v seconds\n", duration)
	start_time := time.Now()
