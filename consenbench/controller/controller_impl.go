...
func (c *Controller) Run(protocol string) {
	...
	cmd = exec.Command("sh", []string{"-c", "mv", filepath.Join(homeDir, "toture-testing-consensus/logs/*.pdf"), filepath.Join(homeDir, "toture-testing-consensus/final-results/"+protocol+"/"+c.Options.Attack)}...)
	output, err = cmd.CombinedOutput()
	if err != nil {
