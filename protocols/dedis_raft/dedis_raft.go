...
func (ba *Dedis_Raft) CopyConsensus(nodes []*common.Node) error {
	...
	config_inputs := []string{"protocols/dedis_raft/assets/config-generate.py", num_replicas, num_clients}
	...
	wg.Wait()
	fmt.Print("Copied the dedis_raft binaries to all the nodes\n")
...
func (ba *Dedis_Raft) Bootstrap(nodes []*common.Node, duration int, result chan util.Performance, bootstrap_complete chan bool) {
	...
	for j := 0; j < int(num_replicas); j++ {
		go func(i int) {
			nodes[i].ExecCmd("." + replica_path + " --name " + strconv.Itoa(i+1) + " --consAlgo raft " + " --viewTimeout " + view_timeout_time + " --pipelineLength " + pipeline_length + " --logFilePath " + fmt.Sprintf("%vbench/logs/", nodes[i].HomeDir) + " --config " + fmt.Sprintf("%vbench/ip_config.yaml", nodes[i].HomeDir))
		}(j)
	}
	...
	wg2.Wait()
	fmt.Println("Downloaded all the dedis_raft client logs")
	...
	for j := int(num_replicas); j < int(num_replicas+num_clients); j++ {
		...
		sshCmd = exec.Command("python3", []string{command, "dedis_raft-" + strconv.Itoa(50+m), logFile}...)
		output, err = sshCmd.CombinedOutput()
		if err != nil {
			print("Error while generating performance graph " + err.Error() + " " + string(output) + "\n")
		} else {
			print("Dedis_Raft Performance graph generated successfully\n" + string(output) + "\n")
		}
	...
	if err != nil {
		...
		print("Dedis_Raft Performance graphs generated successfully\n" + string(output) + "\n")
	}
