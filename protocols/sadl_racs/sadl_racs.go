...
func (ba *Sadl_Racs) CopyConsensus(nodes []*common.Node) error {
	...
	wg.Wait()
	fmt.Print("Copied the sadl_racs binaries to all the nodes\n")
...
func (ba *Sadl_Racs) Bootstrap(nodes []*common.Node, duration int, result chan util.Performance, bootstrap_complete chan bool) {
	...
	wg2.Wait()
	fmt.Println("Downloaded all the sadl_racs client logs")
	...
	for j := int(num_replicas); j < int(num_replicas+num_clients); j++ {
		...
		if err != nil {
			...
			print("Sadl_Racs Performance graph generated successfully\n" + string(output) + "\n")
		}
	...
	if err != nil {
		...
		print("Sadl_Racs Performance graphs generated successfully\n" + string(output) + "\n")
	}
