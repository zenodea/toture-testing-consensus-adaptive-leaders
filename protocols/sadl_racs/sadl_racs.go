...
type Sadl_Racs struct {
	logger  *util.Logger
	options protocols.ConsensusOptions
}

func NewSadl_Racs(logger *util.Logger) *Sadl_Racs {
	return &Sadl_Racs{
		logger: logger,
	}
}

func (ba *Sadl_Racs) CopyConsensus(nodes []*common.Node) error {
	...
	if num_replicas_int+num_clients_int > int64(len(nodes)) {
		panic("Not enough nodes to deploy sadl_racs")
	}

	config_inputs := []string{"protocols/sadl_racs/assets/config-generate.py", num_replicas, num_clients}
	...
	if len(output) > 0 {
		// write the output to protocols/Sadl_Racs/assets/ip_config.yaml
		err = ioutil.WriteFile("protocols/sadl_racs/assets/ip_config.yaml", output, 0644)
		if err != nil {
	...
	for j := int64(0); j < num_clients_int+num_replicas_int; j++ {
		go func(i int) {
			nodes[i].Put_Load("protocols/sadl_racs/assets/ip_config.yaml", fmt.Sprintf("%vbench/", nodes[i].HomeDir))
			nodes[i].Put_Load("protocols/sadl_racs/assets/replica", fmt.Sprintf("%vbench/", nodes[i].HomeDir))
			nodes[i].Put_Load("protocols/sadl_racs/assets/client", fmt.Sprintf("%vbench/", nodes[i].HomeDir))
			wg.Done()
		}(int(j))
	}
	wg.Wait()
	ba.logger.Debug(fmt.Sprintf("Copied the sadl_racs binaries to all the nodes\n"), 0)
...
func (ba *Sadl_Racs) Bootstrap(nodes []*common.Node, duration int, result chan util.Performance, bootstrap_complete chan bool) {
	...
	wg2.Wait()
	ba.logger.Debug(fmt.Sprintf("Downloaded all the sadl_racs client logs"), 0)

	command := "protocols/sadl_racs/assets/performance_graph.py"
	file_names := []string{}
	...
	for j := int(num_replicas); j < int(num_replicas+num_clients); j++ {
		...
		sshCmd = exec.Command("python3", []string{command, "sadl_racs-" + strconv.Itoa(50+m), logFile}...)
		output, err = sshCmd.CombinedOutput()
		if err != nil {
			ba.logger.Debug(fmt.Sprintf("Error while generating performance graph "+err.Error()+" "+string(output)+"\n"), 0)
		} else {
			ba.logger.Debug(fmt.Sprintf("Sadl_Racs Performance graph generated successfully\n"+string(output)+"\n"), 0)
		}

		m++
	}

	sshCmd = exec.Command("python3", append([]string{command, "sadl_racs"}, file_names...)...)
	output, err = sshCmd.CombinedOutput()
	if err != nil {
		ba.logger.Debug(fmt.Sprintf("Error while generating performance graphs "+err.Error()+" "+string(output)+"\n"), 0)
	} else {
		ba.logger.Debug(fmt.Sprintf("Sadl_Racs Performance graphs generated successfully\n"+string(output)+"\n"), 0)
	}

	result <- ba.GetPerformance(clientOutputs)
}

func (ba *Sadl_Racs) ExtractOptions(path string) protocols.ConsensusOptions {
	...
	ba.logger.Debug(fmt.Sprintf("sadl_racs options:\n %v\n", options.Option), 0)

	ba.options = options
	return options
}

func (ba *Sadl_Racs) GetPerformance(outputs []string) util.Performance {
