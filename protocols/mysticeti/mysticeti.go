...
func (ba *Mysticeti) CopyConsensus(nodes []*common.Node) error {
	...
	println("Cloned the mysticeti repository and built the binary")

	nodes[0].Get_Load(fmt.Sprintf("%vmysticeti/target/debug/mysticeti", nodes[0].HomeDir), "protocols/mysticeti/assets/")

	println("Copied the mysticeti binary to the controller machine")
	...
	wg.Wait()
	fmt.Print("Copied the mysticeti binary to all the nodes\n")
	return nil
}
