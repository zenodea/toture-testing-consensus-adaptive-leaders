...
func NewMysticeti(logger *util.Logger) *Mysticeti {
	return &Mysticeti{
		logger: logger,
	}
}

func (ba *Mysticeti) CopyConsensus(nodes []*common.Node) error {
	...
	for j := int64(0); j < num_replicas_int; j++ {
		go func(i int) {
			nodes[i].Put_Load("protocols/mysticeti/assets/mysticeti", fmt.Sprintf("%vbench/", nodes[i].HomeDir))
			nodes[i].Put_Load("protocols/mysticeti/assets/config-rewrite.py", fmt.Sprintf("%vbench/", nodes[i].HomeDir))
			nodes[i].Put_Load("protocols/mysticeti/assets/performance_graph.py", fmt.Sprintf("%vbench/", nodes[i].HomeDir))
			wg.Done()
		}(int(j))
	}
	wg.Wait()
	ba.logger.Debug(fmt.Sprintf("Copied the mysticeti binary to all the nodes\n"), 0)
	return nil
}

func (ba *Mysticeti) Bootstrap(nodes []*common.Node, duration int, result chan util.Performance, bootstrap_complete chan bool) {
	...
	sshCmd := exec.Command("python3", []string{"protocols/mysticeti/assets/genrate-configs.py", "--wave_length", wave_length, "--number_of_leaders", number_of_leaders, "--enable_pipelining", enable_pipelining, "--consensus_only", "true", "--enable_synchronizer", enable_synchronizer, "--initial_delay_secs", "5", "--initial_delay_nanos", "0", "--load", load, "--transaction_size", transaction_size, "--output_dir", "protocols/mysticeti/assets/"}...)
	...
	for i := 0; i < int(num_replicas); i++ {
		go func(j int) {
			perf_result[j] = nodes[j].ExecCmd(fmt.Sprintf("python3 %vbench/performance_graph.py mysticeti  %v %vclient-times-%v.txt", nodes[j].HomeDir, duration, nodes[j].HomeDir, j))
			nodes[j].Get_Load(fmt.Sprintf("%vbench/logs/mysticeti_latency.pdf", nodes[j].HomeDir), fmt.Sprintf("logs/%v_latency.pdf", j))
			nodes[j].Get_Load(fmt.Sprintf("%vbench/logs/mysticeti_throughput.pdf", nodes[j].HomeDir), fmt.Sprintf("logs/%v_throughput.pdf", j))
			wg3.Done()
		}(i)
	}
	wg3.Wait()
...
func (ba *Mysticeti) ExtractOptions(path string) protocols.ConsensusOptions {
	...
	ba.logger.Debug(fmt.Sprintf("Mysticeti options:\n %v\n", options.Option), 0)

	ba.options = options
	return options
}

func (ba *Mysticeti) getPerformance(outputs []string) util.Performance {
	p := util.Performance{
