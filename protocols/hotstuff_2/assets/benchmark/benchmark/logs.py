...
class LogParser:
    ...
    def plot_time_series(self, output_prefix='hotstuff_2'):
        ...
        plt.plot(tps_times_plot, tps_values_plot, label="Hotstuff_2", color='purple')
        plt.xlabel("Time (s)")
        plt.ylabel("Transactions/sec")
        plt.grid(True)
        plt.legend()
        plt.xlim(0, plot_end - plot_start)     # 0--60 on the x-axis
        ...
        # --- Plot latency ---
        plt.figure(figsize=(10, 6))
        plt.plot(latency_times_plot, latency_values_plot, label="Hotstuff_2", color='purple')
        plt.xlabel("Time (s)")
