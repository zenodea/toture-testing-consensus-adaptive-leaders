...
func (ba *Hotstuff_2) CopyConsensus(nodes []*common.Node) error {
	ba.logger.Debug(fmt.Sprintf("Copying Hotstuff_2 consensus to nodes using fabric"), 0)
...
func (ba *Hotstuff_2) Bootstrap(nodes []*common.Node, duration int, result chan util.Performance, bootstrap_complete chan bool) {
	...
	ba.logger.Debug(fmt.Sprintf("Running Hotstuff_2 consensus using fabric"), 0)
	err = os.Chdir("protocols/hotstuff_2/assets/benchmark")
	if err != nil {
		panic("Failed to change directory")
	}
	...
	wg.Wait()
	ba.logger.Debug(fmt.Sprintf("finished running hotstuff"), 0)
...
func (ba *Hotstuff_2) ExtractOptions(path string) protocols.ConsensusOptions {
	...
	ba.logger.Debug(fmt.Sprintf("Hotstuff_2 options:\n %v\n", options.Option), 0)

	ba.options = options
	return options
}

func (ba *Hotstuff_2) GetPerformance(duration int) util.Performance {
	dirPath := "results/"
