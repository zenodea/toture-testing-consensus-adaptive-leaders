...
func (ba *QuePaxa) CopyConsensus(nodes []*common.Node) error {
	...
	wg.Wait()
	fmt.Print("Copied the quepaxa binaries to all the nodes\n")
...
func (ba *QuePaxa) Bootstrap(nodes []*common.Node, duration int, result chan util.Performance, bootstrap_complete chan bool) {
	...
	wg2.Wait()
	fmt.Println("Downloaded all the quepaxa client logs")
	...
	for j := int(num_replicas); j < int(num_replicas+num_clients); j++ {
		...
		if err != nil {
			...
			print("QuePaxa Performance graph generated successfully\n" + string(output) + "\n")
		}
	...
	if err != nil {
		...
		print("QuePaxa Performance graphs generated successfully\n" + string(output) + "\n")
	}
