...
func NewQuePaxa(logger *util.Logger) *QuePaxa {
	return &QuePaxa{
		logger: logger,
	}
}

func (ba *QuePaxa) CopyConsensus(nodes []*common.Node) error {
	...
	if num_replicas_int+num_clients_int > int64(len(nodes)) {
		panic("Not enough nodes to deploy quepaxa")
	}

	config_inputs := []string{"protocols/quepaxa/assets/config-generate.py", num_replicas, num_clients}
	...
	if len(output) > 0 {
		// write the output to protocols/QuePaxa/assets/ip_config.yaml
		err = ioutil.WriteFile("protocols/quepaxa/assets/ip_config.yaml", output, 0644)
		if err != nil {
	...
	for j := int64(0); j < num_clients_int+num_replicas_int; j++ {
		go func(i int) {
			nodes[i].Put_Load("protocols/quepaxa/assets/ip_config.yaml", fmt.Sprintf("%vbench/", nodes[i].HomeDir))
			nodes[i].Put_Load("protocols/quepaxa/assets/replica", fmt.Sprintf("%vbench/", nodes[i].HomeDir))
			nodes[i].Put_Load("protocols/quepaxa/assets/client", fmt.Sprintf("%vbench/", nodes[i].HomeDir))
			wg.Done()
		}(int(j))
	}
	wg.Wait()
	ba.logger.Debug(fmt.Sprintf("Copied the quepaxa binaries to all the nodes\n"), 0)

	return nil
}

func (ba *QuePaxa) Bootstrap(nodes []*common.Node, duration int, result chan util.Performance, bootstrap_complete chan bool) {
	...
	leader_timeout, ok := ba.options.Option["leader_timeout"]
	if !ok {
		panic(err.Error() + " while parsing leader_timeout")
	}
	...
	server_mode, ok := ba.options.Option["server_mode"]
	if !ok {
		panic(err.Error() + " while parsing server_mode")
	}

	leader_mode, ok := ba.options.Option["leader_mode"]
	if !ok {
		panic(err.Error() + " while parsing leader_mode")
	}
	...
	request_propagation_time, ok := ba.options.Option["request_propagation_time"]
	if !ok {
		panic(err.Error() + " while parsing request_propagation_time")
	}
	...
	for j := 0; j < int(num_replicas); j++ {
		go func(i int) {
			nodes[i].ExecCmd("." + replica_path + " --name " + strconv.Itoa(i+1) + " --config " + fmt.Sprintf("%vbench/ip_config.yaml", nodes[i].HomeDir) + " --leaderTimeout " + leader_timeout + " --pipelineLength " + pipeline_length + " --leaderMode " + leader_mode + " --serverMode " + server_mode + " --batchSize " + replica_batch_size + " --batchTime " + replica_batch_time + " --keyLen " + key_len + " --requestPropagationTime " + request_propagation_time + " --valLen  " + val_len)
		}(j)
	}
	...
	nodes[num_replicas].ExecCmd("." + ctl_path + " --name " + strconv.Itoa(51) + " --config " + fmt.Sprintf("%vbench/ip_config.yaml", nodes[num_replicas].HomeDir) + " --logFilePath " + fmt.Sprintf("%vbench/logs/", nodes[num_replicas].HomeDir) + " --requestType status --operationType 1 ")
	...
	for j := int(num_replicas); j < int(num_replicas+num_clients); j++ {
		go func(i int, k int) {
			clientOutputs[i-int(num_replicas)] = nodes[i].ExecCmd("." + ctl_path + " --name " + strconv.Itoa(50+k) + " --config " + fmt.Sprintf("%vbench/ip_config.yaml", nodes[i].HomeDir) + " --logFilePath " + fmt.Sprintf("%vbench/logs/", nodes[i].HomeDir) + " --requestType request --arrivalRate  " + arrival_rate + " --testDuration " + strconv.Itoa(duration) + " --batchSize " + client_batch_size + " --batchTime " + client_batch_time + " --keyLen " + key_len + " --valLen  " + val_len + " --window " + client_window)
		}(j, m)
		m++
	}
	...
	wg2.Wait()
	ba.logger.Debug(fmt.Sprintf("Downloaded all the quepaxa client logs"), 0)

	command := "protocols/quepaxa/assets/performance_graph.py"
	file_names := []string{}
	...
	for j := int(num_replicas); j < int(num_replicas+num_clients); j++ {
		...
		sshCmd = exec.Command("python3", []string{command, "quepaxa-" + strconv.Itoa(50+m), logFile}...)
		output, err = sshCmd.CombinedOutput()
		if err != nil {
			ba.logger.Debug(fmt.Sprintf("Error while generating performance graph "+err.Error()+" "+string(output)+"\n"), 0)
		} else {
			ba.logger.Debug(fmt.Sprintf("QuePaxa Performance graph generated successfully\n"+string(output)+"\n"), 0)
		}

		m++
	}

	sshCmd = exec.Command("python3", append([]string{command, "quepaxa"}, file_names...)...)
	output, err = sshCmd.CombinedOutput()
	if err != nil {
		ba.logger.Debug(fmt.Sprintf("Error while generating performance graphs "+err.Error()+" "+string(output)+"\n"), 0)
	} else {
		ba.logger.Debug(fmt.Sprintf("QuePaxa Performance graphs generated successfully\n"+string(output)+"\n"), 0)
	}

	result <- ba.GetPerformance(clientOutputs)
}

func (ba *QuePaxa) ExtractOptions(path string) protocols.ConsensusOptions {
	...
	ba.logger.Debug(fmt.Sprintf("quepaxa options:\n %v\n", options.Option), 0)

	ba.options = options
	return options
}

func (ba *QuePaxa) GetPerformance(outputs []string) util.Performance {
