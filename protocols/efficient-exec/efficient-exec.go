...
type EfficientExec struct {
	logger  *util.Logger
	options protocols.ConsensusOptions
}

func NewEfficientExec(logger *util.Logger) *EfficientExec {
	return &EfficientExec{
		logger: logger,
	}
}

func (ba *EfficientExec) CopyConsensus(nodes []*common.Node) error {
...
func (ba *EfficientExec) Bootstrap(nodes []*common.Node, duration int, result chan util.Performance, bootstrap_complete chan bool) {
...
func (ba *EfficientExec) ExtractOptions(path string) protocols.ConsensusOptions {
...
func (ba *EfficientExec) GetPerformance(outputs []string) util.Performance {
