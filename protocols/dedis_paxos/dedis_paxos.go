...
func (ba *Dedis_Paxos) CopyConsensus(nodes []*common.Node) error {
	...
	config_inputs := []string{"protocols/dedis_paxos/assets/config-generate.py", num_replicas, num_replicas}
	...
	wg.Wait()
	ba.logger.Debug(fmt.Sprintf("Copied the dedis_paxos binaries to all the nodes\n"), 0)
...
func (ba *Dedis_Paxos) Bootstrap(nodes []*common.Node, duration int, result chan util.Performance, bootstrap_complete chan bool) {
	...
	for j := 0; j < int(num_replicas); j++ {
		go func(i int) {
			nodes[i].ExecCmd("." + replica_path + " --name " + strconv.Itoa(i+1) + " --consAlgo paxos " + " --viewTimeout " + view_timeout_time + " --pipelineLength " + pipeline_length + " --logFilePath " + fmt.Sprintf("%vbench/logs/", nodes[i].HomeDir) + " --config " + fmt.Sprintf("%vbench/ip_config.yaml", nodes[i].HomeDir) + "  --batchSize " + replica_batch_size + "  --batchTime " + replica_batch_time + "  --keyLen " + key_len + "  --valLen " + val_len)
		}(j)
	}
	...
	//}
	//wg2.Wait()
	//ba.logger.Debug(fmt.Sprintf("Downloaded all the dedis_paxos client logs"), 0)
	//
	...
	//} else {
	//	ba.logger.Debug(fmt.Sprintf("Dedis_Paxos Performance graphs generated successfully\n"+string(output)+"\n"), 0)
	//}
...
func (ba *Dedis_Paxos) ExtractOptions(path string) protocols.ConsensusOptions {
	...
	ba.logger.Debug(fmt.Sprintf("dedis_paxos options:\n %v\n", options.Option), 0)
